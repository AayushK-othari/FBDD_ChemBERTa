...
# ───────────────────────── LOAD DATA ─────────────────────
train_df = pd.read_csv("mTORcanonical_unique_drug_train.csv")
val_df = pd.read_csv("mTORcanonical_unique_drug_val.csv")
test_df = pd.read_csv("mTORcanonical_unique_drug_test.csv")
...
class SMILESDataset(Dataset):
    ...
    def __getitem__(self, idx):
        drug = self.df.iloc[idx]["DRUG_SMILES"]
        frag = self.df.iloc[idx]["BEST_FRAGMENT"]
        if not drug:
            self.skipped += 1
            return None
...
optimizer = optim.AdamW(model.parameters(), lr=5e-5, weight_decay=4e-5)
...
for ep in range(NUM_EPOCHS):
    print("Sample SMILES:", train_df.sample(1)["DRUG_SMILES"].values[0])
    model.train()
    ep_losses, ep_sims = [], []
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {ep + 1}/{NUM_EPOCHS}", leave=False):
        ...
        sims = [tanimoto_similarity(canonical(t), canonical(p)) for p, t in zip(pred_smi, true_smi)]
        ep_sims.extend(sims)
    ...
    if mean_val_tan > best_val_tani + 5e-3:
